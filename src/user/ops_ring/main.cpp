...
void general()
{
    ...
    //-  static_cast
    {
        const T * z = &x;
        x = *const_cast<T *>(z);
        auto q1 = static_cast<T>(x);
        std::ignore = reinterpret_cast<T *>(&x);
        std::ignore = q1;
    }
...
void special()
{
    cout << "Names<T> (typ,fil,cls) : " << T::typname() << ' '
         << T::filname() << ' ' << T::clsname() << '\n';

    cout << "Traits (sign,size,circ) : " << T::is_signed << ' '
         << T::size << ' ' << T::is_circuit << '\n';
    ...
    cout << "Names(x) (typ,fil,cls) : " << x.typname() << ' ' << x.filname() << ' ' << x.clsname() << '\n';
